...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        #print('Shift scores shape:', shift_scores.shape)
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    # Forward pass: calculate scores and apply the numerical stability trick
    scores = X.dot(W)
    ...
    correct_log_probs = -np.log(softmax_output[np.arange(num_train), y] + 1e-8)  # Adding an epsilon for numerical stability
    ...
    dS /= num_train
    dW = X.T.dot(dS)  # Gradient w.r.t. weights
